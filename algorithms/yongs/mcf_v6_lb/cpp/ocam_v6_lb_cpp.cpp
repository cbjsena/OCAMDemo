...
constexpr const char* SAIL_COST_LB_YEAR_MONTH = "__LB_MIN_BUNKER_MONTH__";
constexpr const char* VIRTUAL_VESSEL_CODE = "v_0";
constexpr bool INCLUDE_VIRTUAL_VESSEL = false;
...
bool is_ts_label(const std::string& label) {
    return label == "ts_in_before" || label == "ts_in_after" || label == "ts_out_before" || label == "ts_out_after";
}
...
struct Data {
    ...
    std::string model_name = "mcf_v6_lb";
    ...
    std::map<std::pair<std::string, std::string>, std::vector<double>> bunker_prices_by_year_month_type;
...
struct Builder {
    ...
    long long minimum_sail_seconds(const std::string& from_port, const std::string& to_port) const {
        return static_cast<long long>(std::llround(lookup_distance(from_port, to_port) / 20.0 * HOUR));
    }

    void relax_ts_node_times_for_lane(const std::vector<int>& key_items) {
        // v6_lb keeps the same TS timing relaxation as the lower-bound model; only the
        // out-lane sail candidate set is enlarged by canal-route alternatives.
        std::vector<int> group_indices;
        for (int item_index : key_items) {
            const Item& item = items[item_index];
            if (item.kind == ItemKind::Group) group_indices.push_back(item.group_index);
        }
        for (size_t index = 0; index < group_indices.size(); ++index) {
            Group& group = groups[group_indices[index]];
            if (group.ts_in_after >= 0 && index + 1 < group_indices.size()) {
                const Group& next_group = groups[group_indices[index + 1]];
                long long min_sail = minimum_sail_seconds(group.event.port_code, next_group.event.port_code);
                nodes[group.ts_in_after].node_in = std::max(
                    next_group.event.pilot_in_start - min_sail - 6 * HOUR,
                    group.event.pilot_out_end + 30 * HOUR
                );
            }
            if (group.ts_out_before >= 0 && index > 0) {
                const Group& prev_group = groups[group_indices[index - 1]];
                long long min_sail = minimum_sail_seconds(prev_group.event.port_code, group.event.port_code);
                nodes[group.ts_out_before].node_out = std::min(
                    prev_group.event.pilot_out_end + min_sail + 6 * HOUR,
                    group.event.pilot_in_start - 30 * HOUR
                );
            }
        }
    }
    ...
    void build_lane_nodes() {
        ...
        for (const Version& version : data.versions) {
            ...
            for (int position_no : pos_it->second) {
                ...
                relax_ts_node_times_for_lane(key_items);
                if (!key_items.empty()) {
...
struct Optimizer {
    ...
    mutable std::map<std::tuple<int, std::string, std::string, std::string>, CanalRouteChoice> canal_bunker_choice_cache;
    ...
    std::vector<std::string> complete_bunker_year_months() const {
        std::set<std::string> mgo_months;
        std::set<std::string> lsfo_months;
        for (const auto& entry : data.bunker_prices_by_year_month_type) {
            if (entry.first.second == "MGO") mgo_months.insert(entry.first.first);
            if (entry.first.second == "LSFO") lsfo_months.insert(entry.first.first);
        }
        std::vector<std::string> months;
        for (const std::string& ym : mgo_months) {
            if (lsfo_months.count(ym)) months.push_back(ym);
        }
        if (months.empty()) throw std::runtime_error("no year-month has both MGO and LSFO bunker prices");
        return months;
    }

    std::vector<std::string> year_months_in_interval(long long start, long long end) const {
        if (end < start) throw std::runtime_error("empty year-month interval");
        auto to_year_month_pair = [](long long epoch_seconds) {
            std::time_t raw = static_cast<std::time_t>(epoch_seconds);
            std::tm tm_value{};
#if defined(_WIN32)
            gmtime_s(&tm_value, &raw);
            ...
            gmtime_r(&raw, &tm_value);
#endif
            return std::make_pair(tm_value.tm_year + 1900, tm_value.tm_mon + 1);
        };
        auto [year, month] = to_year_month_pair(start);
        auto [end_year, end_month] = to_year_month_pair(end);
        std::vector<std::string> months;
        while (year < end_year || (year == end_year && month <= end_month)) {
            std::ostringstream ym;
            ym << year << std::setw(2) << std::setfill('0') << month;
            months.push_back(ym.str());
            if (++month == 13) {
                month = 1;
                ++year;
            }
    ...
    double min_transshipment_cost(long long start, long long end, const std::string& lane_code, const std::string& port_code) const {
        double best = std::numeric_limits<double>::infinity();
        std::vector<std::string> missing;
        for (const std::string& ym : year_months_in_interval(start, end)) {
            auto it = data.transshipment_cost_by_key.find({ym, lane_code, port_code});
            if (it == data.transshipment_cost_by_key.end()) {
                missing.push_back(ym);
                continue;
            }
        ...
        if (!missing.empty()) {
            ...
            message << "missing transshipment cost for lower-bound TS window lane_code=" << lane_code
                    << ", port_code=" << port_code << ", year_month=";
        ...
        if (!std::isfinite(best)) throw std::runtime_error("empty transshipment cost lower-bound window");
        return best;
    }

    double sail_cost_for_capacity_month(
        ...
        if (dist_it == data.distance_info_by_port_pair.end()) {
            throw std::runtime_error(
                "missing distance info for from_port_code=" + from_port +
    ...
    double sail_cost_for_capacity(
        ...
        if (ym != SAIL_COST_LB_YEAR_MONTH) {
            return sail_cost_for_capacity_month(capacity, lane_code, ym, from_port, to_port, distance, sail_time);
        }
        double best = std::numeric_limits<double>::infinity();
        for (const std::string& candidate_ym : complete_bunker_year_months()) {
            best = std::min(
                best,
                sail_cost_for_capacity_month(capacity, lane_code, candidate_ym, from_port, to_port, distance, sail_time)
            );
        }
        return best;
    }
    ...
    CanalRouteChoice canal_bunker_choice_for_capacity_month(
        const ModelEdge& edge,
        int capacity,
        const std::string& lane_code,
        const std::string& ym
    ) const {
        auto cache_key = std::make_tuple(capacity, lane_code, edge.edge_id, ym);
    ...
    CanalRouteChoice canal_bunker_choice_for_capacity(
        ...
        if (edge.sail_year_month != SAIL_COST_LB_YEAR_MONTH) {
            return canal_bunker_choice_for_capacity_month(edge, capacity, lane_code, edge.sail_year_month);
        }
        CanalRouteChoice best;
        best.total_cost = std::numeric_limits<double>::infinity();
        for (const std::string& ym : complete_bunker_year_months()) {
            CanalRouteChoice choice = canal_bunker_choice_for_capacity_month(edge, capacity, lane_code, ym);
            if (choice.total_cost < best.total_cost - 1e-9) best = choice;
        }
        return best;
    }
                ...
                try {
                    for (size_t arc_index = begin; arc_index < end; ++arc_index) {
                        ...
                        if (arc.type == "SailArc" || arc.type == "HorizonSailArc" || arc.type == "CanalSailArc") {
                            ...
                            if (is_ts_label(from_node.label) || is_ts_label(to_node.label)) {
                                edge.sail_year_month = SAIL_COST_LB_YEAR_MONTH;
                            }
                            if (arc.type != "CanalSailArc") {
                                if (!data.distance_info_by_port_pair.count({edge.sail_from_port, edge.sail_to_port})) {
                                    throw std::runtime_error(
                                        "missing distance info for sail edge_id=" + edge.edge_id +
                                        ", from_port_code=" + edge.sail_from_port +
                                        ", to_port_code=" + edge.sail_to_port
                                    );
                                }
    ...
    void prepare_ts_units() {
        for (const Group& group : builder.groups) {
            auto add_unit = [&](const std::string& suffix, int tsi, int tso) {
                ...
                const Node& tsi_node = builder.nodes[tsi];
                const Node& tso_node = builder.nodes[tso];
                long long start = std::max(
                    std::min(std::min(tsi_node.node_in, tsi_node.node_out), std::min(tso_node.node_in, tso_node.node_out - 6 * HOUR)),
                    data.planning_start
                );
                long long end = std::min(
                    std::max(std::max(tsi_node.node_in, tsi_node.node_out), std::max(tso_node.node_in, tso_node.node_out)),
                    data.planning_end
                );
                if (start > end) throw std::runtime_error("TS cost window does not intersect planning horizon");
                unit.cost = min_transshipment_cost(start, end, unit.lane_code, unit.port_code);
...
Data load_data(const std::string& bundle_dir) {
    ...
    for (const auto& row : read_tsv(bundle_dir + "/bunker_price.tsv")) {
        ...
        data.bunker_prices_by_year_month_type[{row[0], row[2]}].push_back(price);
    }
...
extern "C" int ocam_v6_lb_construct_network(const char* bundle_dir, char* out, std::size_t out_cap, char* err, std::size_t err_cap) {
    try {
...
extern "C" int ocam_v6_lb_solve_flow(const char* bundle_dir, char* out, std::size_t out_cap, char* err, std::size_t err_cap) {
    try {
        std::string bundle_path(bundle_dir);
        Builder builder(load_data(bundle_path));
...
extern "C" int ocam_v6_lb_gurobi_smoke(char* out, std::size_t out_cap, char* err, std::size_t err_cap) {
    try {
        GRBEnv env(true);
