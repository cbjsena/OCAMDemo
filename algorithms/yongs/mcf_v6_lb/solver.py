...
from ocam.models import DeclaredPosition, InstanceData, LowerBoundResult
from .cpp_backend import solve_flow_cpp

DESCRIPTION = "v6_lb: TS timing 완화, lower-envelope 비용, 운하 경유 선택지를 포함한 full C++ MCF 하한 모형을 최적화한다."
...
def _split_dry_dock_vessels(
    ...
    if split_codes_by_original:
        print(
            "[mcf_v6_lb] split dry-dock vessels "
...
def algorithm(instance_data: InstanceData, timelimit: int) -> LowerBoundResult:
    ...
    LAST_MULTICOMMODITY_RESULT = solve_flow_cpp(
        working_instance_data,
        network_positions,
        output_path="/private/tmp/ocam_yongs/mcf_v6_lb_mip_paths.html",
        model_name="mcf_v6_lb",
        dd_couplings=dd_couplings,
    )
    network = LAST_MULTICOMMODITY_RESULT["network"]
    log_stage(
        "cpp flow solve finished "
        f"nodes={network.number_of_nodes()} edges={network.number_of_edges()} "
        f"status={LAST_MULTICOMMODITY_RESULT.get('status_name', LAST_MULTICOMMODITY_RESULT.get('status'))}"
    )

    lower_bound = float(LAST_MULTICOMMODITY_RESULT["objective_value"])
    objective = {
        "lower_bound": lower_bound,
        "objective_value": lower_bound,
    }
    for key in ("objective_bound", "mip_gap", "status", "status_name", "sol_count", "model_name"):
        if key in LAST_MULTICOMMODITY_RESULT:
            objective[key] = LAST_MULTICOMMODITY_RESULT[key]
    log_stage(f"prepared lower-bound result value={lower_bound}")
    return LowerBoundResult(
        algorithm="yongs/mcf_v6_lb",
        lower_bound=lower_bound,
        objective=objective,
        metadata={
            "bound_valid_for": "exact_problem",
            "relaxation": "mcf_v6_lb",
            "result_semantics": "lower_bound_only",
        },
    )
