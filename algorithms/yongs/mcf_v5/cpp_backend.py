...
LIB_PATH = CPP_DIR / "libocam_v5_cpp.so"
...
def _ensure_cpp_library() -> None:
    source = CPP_DIR / "ocam_v5_cpp.cpp"
...
def construct_network_cpp(
    ...
    _ensure_cpp_library()
    with tempfile.TemporaryDirectory(prefix="ocam_v5_") as temp_dir:
        bundle_dir = Path(temp_dir)
        _export_network_input_bundle(instance_data, network_positions, bundle_dir)
        lib = ctypes.CDLL(str(LIB_PATH))
        lib.ocam_v5_construct_network.argtypes = [
            ctypes.c_char_p,
            ctypes.c_char_p,
            ctypes.c_size_t,
            ctypes.c_char_p,
        ...
        lib.ocam_v5_construct_network.restype = ctypes.c_int
        out = ctypes.create_string_buffer(4096)
        err = ctypes.create_string_buffer(4096)
        code = lib.ocam_v5_construct_network(str(bundle_dir).encode(), out, len(out), err, len(err))
        if code != 0:
...
def solve_flow_cpp(
    ...
    _ensure_cpp_library()
    with tempfile.TemporaryDirectory(prefix="ocam_v5_solve_") as temp_dir:
        bundle_dir = Path(temp_dir)
        _export_network_input_bundle(instance_data, network_positions, bundle_dir, dd_couplings, model_name)
        lib = ctypes.CDLL(str(LIB_PATH))
        lib.ocam_v5_solve_flow.argtypes = [
            ctypes.c_char_p,
            ctypes.c_char_p,
            ctypes.c_size_t,
            ctypes.c_char_p,
        ...
        lib.ocam_v5_solve_flow.restype = ctypes.c_int
        out = ctypes.create_string_buffer(4096)
        err = ctypes.create_string_buffer(4096)
        code = lib.ocam_v5_solve_flow(str(bundle_dir).encode(), out, len(out), err, len(err))
        if code != 0:
            raise RuntimeError(err.value.decode())
