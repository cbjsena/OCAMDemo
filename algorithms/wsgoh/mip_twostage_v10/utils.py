...
def _load_gurobi():
    try:
        ...
        raise RuntimeError(
            "wsgoh/mip_twostage_v10 requires gurobipy at runtime. "
...
def _build_coverage_context(instance_data: InstanceData) -> CoverageContext:
    ...
    for position_key in fixed_positions:
    ...
    return CoverageContext(
        ...
        initial_selectable_values={position_key: 0 for position_key in selectable_positions},
...
def _coverage_impossibility_summary(context: CoverageContext, patterns: list[Pattern]) -> str:
    ...
    fixed_missing = [key for key in context.fixed_coverage if patterns_by_coverage.get(key, 0) == 0]
    selectable_positions_without_any = []
    for position_key, schedule in context.selectable_schedules.items():
