...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run wsgoh/mip_twostage_v10 directly. OCAM's normal entrypoint is "
...
def _resolve_direct_run_arguments(namespace: argparse.Namespace) -> DirectRunArguments:
    ...
    if output_solution is None:
        ...
        output_solution = _make_timestamped_output_dir(base_output_dir) / "wsgoh_mip_twostage_v10_solution.json"
    else:
...
def main(algorithm_func: Callable[[Any, int], CascadingSolution]) -> None:
    ...
    if args.output_solution is not None:
        ...
        print(f"wsgoh/mip_twostage_v10 wrote solution to {args.output_solution}")
