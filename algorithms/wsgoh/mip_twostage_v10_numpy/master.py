...
def _build_master_model(
    ...
    *,
    ...
    if not patterns:
        raise ValueError("wsgoh/mip_twostage_v10_numpy: pattern pool is empty.")
    if not context.required_coverage:
        raise ValueError("wsgoh/mip_twostage_v10_numpy: no required coverage keys were generated.")
    ...
    if solve_mode == "norel":
        ...
        raise ValueError(f"wsgoh/mip_twostage_v10_numpy: unknown solve mode {solve_mode!r}.")
    ...
    for pattern in patterns:
        ...
        if missing_requirements:
            raise ValueError(
                f"wsgoh/mip_twostage_v10_numpy: pattern {pattern.pattern_id!r} has missing parent patterns "
    ...
    for vessel in instance_data.vessels:
        ...
        if not vessel_pattern_ids:
            raise ValueError(f"wsgoh/mip_twostage_v10_numpy: missing actual-vessel pattern for {vessel_code!r}.")
        model.addConstr(
...
def _build_phase_i_master_model(
    ...
    *,
    ...
    if not patterns:
        raise ValueError("wsgoh/mip_twostage_v10_numpy: Phase-I pattern pool is empty.")
    if any(pattern.is_virtual for pattern in patterns):
        raise ValueError("wsgoh/mip_twostage_v10_numpy: Phase-I actual-only pool contains virtual patterns.")
    if not context.required_coverage:
        raise ValueError("wsgoh/mip_twostage_v10_numpy: no required coverage keys were generated.")
...
def _solve_actual_master(
    ...
    model, y, z = _build_master_model(
        gp,
        GRB,
        model_name=f"wsgoh_mip_twostage_v10_numpy_actual_{_safe_name(iteration_label)}",
...
def _solve_phase_i_actual_master(
    ...
    model, y, z, miss, extra = _build_phase_i_master_model(
        gp,
        GRB,
        model_name="wsgoh_mip_twostage_v10_numpy_phase_i_actual",
        instance_data=instance_data,
        context=context,
        patterns=patterns,
        mip_timelimit=mip_timelimit,
...
def _solve_fallback_master(
    ...
    model, y, z = _build_master_model(
        gp,
        GRB,
        model_name="wsgoh_mip_twostage_v10_numpy_mixed_improve",
        instance_data=instance_data,
        context=context,
        patterns=patterns,
        mip_timelimit=mip_timelimit,
